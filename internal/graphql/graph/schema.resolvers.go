...
type Post interface {
	SavePost(ctx context.Context) (string, error)
	GetAll(ctx context.Context) ([]posts.Posts, error)
	GetPost(ctx context.Context, id string) (*posts.Posts, error)
}
...
func (r *mutationResolver) CreatePost(ctx context.Context, title string, content string, commentsAllowed bool) (*model.Post, error) {
	...
	ps.Title = title
	ps.Content = content
	ps.CommentsAllowed = commentsAllowed
	ps.ID = uuid.New().String()

	postId, err := ps.SavePost(ctx)
	if err != nil {
		r.Logger.Error("Failed to save post", err, op)
		return nil, fmt.Errorf("%s: failed to update balance: %w", op, err)
	}
	return &model.Post{ID: postId, Title: ps.Title, Content: ps.Content, CommentsAllowed: ps.CommentsAllowed}, nil
}
...
func (r *mutationResolver) CreateComment(ctx context.Context, postID string, parentID *string, text string) (*model.Comment, error) {
	...
	var err error
	post, err := posts.GetPost(ctx, postID)
	if err != nil {
	...
	com.PostID = postID
	if parentID != nil {
		err = comments.CheckCommentId(ctx, *parentID)
		if err != nil {
	...
	CommentId, err := com.SaveComment(ctx)
	if err != nil {
...
func (r *queryResolver) Post(ctx context.Context, id string, first *int32, after *string) (*model.Post, error) {
	...
	post, err := posts.GetPost(ctx, id)
	if err != nil {
		r.Logger.Error("Failed to get post", slog.String("postID", id), err, op)
		return nil, fmt.Errorf("%s: failed to get posts: %w", op, err)
	}
	dbComms, endCursor, hasNextpage, err := comments.GetComments(ctx, id, first, after)
	if err != nil {
